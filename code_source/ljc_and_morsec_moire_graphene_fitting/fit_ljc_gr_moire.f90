...
integer					:: ar_c_num(2)
character(len=256)		:: ar_settings_filename(2),output_prefix,input_path,out_path,&
						   ar_final_file(2),param_file,ar_start_xyz_file(2),ar_xyz_file(2)						
real 					:: z,ar_zero_energy_level(2),be0,ar_grd0(2),ljR(2)
logical					:: ljsimplified
character(len=80)		:: line

contains

subroutine calc_error(error,from_init_xyz,ljp)
character(len=256)		::  settings_filename,start_xyz_file,xyz_file,final_file,str,tempstring,op,prevop
real 					::	ljp(3),error,arr1(3),arr2(3),be,bd,grd
...
integer					::	i,rnm_err
	...
	sim_num = sim_num+1
	write(out_id,*)
	write(out_id,*) sim_num,ljp(3),ljp(1),ljp(2)
	
	open(1234,file=trim(input_path)//trim(param_file))
	write(1234,*) ljp(3),ljp(1),ljp(2)
	write(1234,*) ljR(1),ljR(2)
	write(1234,*) ljsimplified
	close(1234)
	
	write(oid,'(i6,3f21.6)',advance='no') sim_num,ljp(3),ljp(1),ljp(2)
...
subroutine set_fitting_parameters(fitting_parameters_file_name,init_min_ljp,init_max_ljp)
real 					:: init_min_ljp(3),init_max_ljp(3)
...
integer					:: i
	...
	read(9,'(A16,A)') str,input_path;				write(out_id,*) trim(input_path)
	read(9,'(A16,A)') str,out_path;					write(out_id,*) trim(out_path)
	read(9,*) str,output_prefix;			write(out_id,*) trim(output_prefix)
	read(9,*) str,ar_settings_filename(1);	write(out_id,*) trim(ar_settings_filename(1))
	read(9,*) str,ar_start_xyz_file(1);		write(out_id,*) trim(ar_start_xyz_file(1))
	read(9,*) str,ar_c_num(1);				write(out_id,*) ar_c_num(1)
	read(9,*) str,ar_zero_energy_level(1);	write(out_id,*) ar_zero_energy_level(1)
	read(9,*) str,ar_final_file(1);			write(out_id,*) trim(ar_final_file(1))
	read(9,*) str,ar_settings_filename(2);	write(out_id,*) trim(ar_settings_filename(2))
	read(9,*) str,ar_start_xyz_file(2);		write(out_id,*) trim(ar_start_xyz_file(2))
	read(9,*) str,ar_c_num(2);				write(out_id,*) ar_c_num(2)
	read(9,*) str,ar_zero_energy_level(2);	write(out_id,*) ar_zero_energy_level(2)
	read(9,*) str,ar_final_file(2);			write(out_id,*) trim(ar_final_file(2))
	read(9,*) str,min_param_file;			write(out_id,*) trim(min_param_file)
	read(9,*) str,max_param_file;			write(out_id,*) trim(max_param_file)
	read(9,*) str,z;						write(out_id,*) z
	read(9,*) str,be0;						write(out_id,*) be0
	read(9,*) str,ar_grd0(1);				write(out_id,*) ar_grd0(1)
	read(9,*) str,ar_grd0(2);				write(out_id,*) ar_grd0(2)
	close(9)
	
	open(1234,file=trim(input_path)//trim(min_param_file))
	read(1234,*) init_min_ljp(3),init_min_ljp(1),init_min_ljp(2)
	read(1234,*) ljR(1),ljR(2)
	close(1234)
	open(1234,file=trim(input_path)//trim(max_param_file))
	read(1234,*) init_max_ljp(3),init_max_ljp(1),init_max_ljp(2)
	close(1234)
	ljsimplified = .false.
